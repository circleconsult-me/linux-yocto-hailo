...
enum xspi_config_id {
	CONFIG_ID_XSPI_DEFAULT = 0x0,
	CONFIG_ID_XSPI_HAILO15,
};
...
struct cdns_xspi_dev {
	...
	u8 hw_num_banks;
	enum xspi_config_id cfg_id;
};

struct xspi_config_data {
	enum xspi_config_id cfg_id;
};
...
static irqreturn_t cdns_xspi_irq_handler(int this_irq, void *dev)
{
	...
	if (result == IRQ_HANDLED && cdns_xspi->cfg_id == CONFIG_ID_XSPI_HAILO15) {
		writel(XSPI_HAILO_WRAPPER_IRQ_CLEAR_VALUE, cdns_xspi->wrapperbase + XSPI_HAILO_WRAPPER_IRQ_CLEAR_OFFSET);
	}
...
static const struct xspi_config_data default_config = {
	.cfg_id = CONFIG_ID_XSPI_DEFAULT,
};
...
static int cdns_xspi_probe(struct platform_device *pdev)
{
	...
	struct resource *res;
	const struct xspi_config_data *match;
	int ret;
	...
	match = (struct xspi_config_data*)of_device_get_match_data(&pdev->dev);
	if (match) {
		cdns_xspi->cfg_id = match->cfg_id;
	}
	else {
		cdns_xspi->cfg_id = default_config.cfg_id;
	}
	...
	if (cdns_xspi->cfg_id == CONFIG_ID_XSPI_HAILO15) {
		cdns_xspi->wrapperbase = devm_platform_ioremap_resource_byname(pdev, "wrapper");
		if (IS_ERR(cdns_xspi->wrapperbase)) {
...
static const struct xspi_config_data hailo15_config = {
	.cfg_id = CONFIG_ID_XSPI_HAILO15,
};

static const struct of_device_id cdns_xspi_of_match[] = {
	{ .compatible = "cdns,xspi-nor"},
	{ .compatible = "hailo,xspi-nor", .data = &hailo15_config },
	{ /* end of table */}
};
